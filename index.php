...
const bulkSubmissionForm=document.querySelector('.js-submission-bulk');if(bulkSubmissionForm){const scope=bulkSubmissionForm.closest('.ios-surface,.submission-page')||document;const boxes=()=>[...scope.querySelectorAll('.js-submission-select')];const master=scope.querySelector('.js-submission-select-all');const countNode=bulkSubmissionForm.querySelector('.js-submission-selected-count');const buttons=[...scope.querySelectorAll('.js-bulk-submit')];const deleteModal=document.getElementById('bulkSubmissionDeleteModal');const deleteCount=scope.querySelector('.js-bulk-delete-count')||document.querySelector('.js-bulk-delete-count');const selectedCount=()=>boxes().filter(x=>x.checked).length;const syncBulk=()=>{const count=selectedCount();if(countNode)countNode.textContent=String(count);if(deleteCount)deleteCount.textContent=String(count);buttons.forEach(x=>x.disabled=count===0);if(master){const total=boxes().length;master.checked=total>0&&count===total;master.indeterminate=count>0&&count<total;master.setAttribute('aria-checked',master.indeterminate?'mixed':(master.checked?'true':'false'));}};const toggleAll=checked=>{boxes().forEach(x=>{x.checked=checked;});syncBulk();};master?.addEventListener('change',()=>toggleAll(!!master.checked));master?.closest('th,td,div,label')?.addEventListener('click',e=>{if(e.target===master)return;e.preventDefault();toggleAll(!(master.checked&&!master.indeterminate));});scope.addEventListener('change',e=>{if(!e.target.classList.contains('js-submission-select'))return;syncBulk();});bulkSubmissionForm.addEventListener('submit',e=>{if(selectedCount()>0)return;e.preventDefault();alert(bulkSubmissionForm.dataset.emptyMessage||'Select at least one item');});deleteModal?.addEventListener('show.bs.modal',e=>{const count=selectedCount();if(!count){e.preventDefault();return;}if(deleteCount)deleteCount.textContent=String(count);});syncBulk();}
...
function layout($html,$cid=0,$show=null){
    ...
    echo $html.'</section></div></main>'.projects_modal().($context?collections_modal($cid).collection_modal(null,'collectionNewModal',$defaultGroup).collection_section_action_modals_all():'').($editCollection?collection_modal($editCollection,'collectionEditModal'):'').clean_files_modal().universal_delete_modal().language_disable_modal();$show=$show?:old_modal();if(!$show&&isset($_GET['new_col']))$show='collectionNewModal';if(!$show&&isset($_GET['edit_col']))$show='collectionEditModal';$show=$show?:((isset($_GET['project_edit']))?'projectModal':null);foot($show);old_clear();
}
function collection_modal($c=null,$mid='collectionModal',$defaultGroup=0){
    ...
    $body='<input type="hidden" name="id" value="'.$id.'">';if($id)$body.='<input type="hidden" name="_sync_sections" value="1"><input type="hidden" name="_return" value="'.h(stable_return_url($_SERVER['REQUEST_URI']??U(['collections'=>1]))).'">';
...
function dashboardPage(){
    $pid=current_project_id();$project=current_project();$stats=project_stats();$recent=recent_entries_get();$favIds=favorite_ids();$favs=[];foreach($favIds as $id)if($c=col($id))$favs[]=$c;
    $sections=(int)q('SELECT COUNT(*) FROM g WHERE pid=?',[$pid])->fetchColumn();
    $forms=(int)q('SELECT COUNT(*) FROM forms WHERE pid=?',[$pid])->fetchColumn();
    $submissions=(int)q('SELECT COUNT(*) FROM form_submissions WHERE pid=?',[$pid])->fetchColumn();
    $fileBytes=(int)q("SELECT COALESCE(SUM(sz),0) FROM files WHERE pid=? AND st='active'",[$pid])->fetchColumn();
    $privateCollections=(int)q("SELECT COUNT(*) FROM c WHERE pid=? AND access_mode='private'",[$pid])->fetchColumn();
    $privateSections=(int)q("SELECT COUNT(*) FROM g WHERE pid=? AND access_mode='private'",[$pid])->fetchColumn();
    $privateForms=(int)q("SELECT COUNT(*) FROM forms WHERE pid=? AND access_mode='private'",[$pid])->fetchColumn();
    $activeForms=(int)q("SELECT COUNT(*) FROM forms WHERE pid=? AND st='active'",[$pid])->fetchColumn();
    $inactiveForms=max(0,$forms-$activeForms);
    $submissionNew=(int)q("SELECT COUNT(*) FROM form_submissions WHERE pid=? AND st='new'",[$pid])->fetchColumn();
    $submissionRead=(int)q("SELECT COUNT(*) FROM form_submissions WHERE pid=? AND st='read'",[$pid])->fetchColumn();
    $submissionSpam=(int)q("SELECT COUNT(*) FROM form_submissions WHERE pid=? AND st='spam'",[$pid])->fetchColumn();
    $collectionsWithEntries=(int)q('SELECT COUNT(*) FROM c WHERE pid=? AND EXISTS(SELECT 1 FROM e WHERE e.cid=c.id)',[$pid])->fetchColumn();
    $topCollections=all('SELECT c.id,c.n,c.s,c.m,c.access_mode,COUNT(e.id) AS entry_count FROM c LEFT JOIN e ON e.cid=c.id WHERE c.pid=? GROUP BY c.id,c.n,c.s,c.m,c.access_mode ORDER BY entry_count DESC,c.n ASC LIMIT 5',[$pid]);
...
function group_workspace_header($g,$endpoint,$count){
    $gid=(int)$g['id'];
    $back='<a class="btn btn-light collection-workspace-back" href="'.h(U(['groups'=>1])).'" aria-label="'.h(T('back')).'">'.icon('arrow-left').'</a>';
...
function formsPage(){
    ...
    $rows=all('SELECT f.*,(SELECT COUNT(*) FROM form_fields ff WHERE ff.fid=f.id AND ff.pid=f.pid) AS field_count,(SELECT COUNT(*) FROM form_submissions x WHERE x.fid=f.id AND x.pid=f.pid) AS submission_count,(SELECT MAX(ca) FROM form_submissions x WHERE x.fid=f.id AND x.pid=f.pid) AS last_submission FROM forms f WHERE '.$where.' ORDER BY '.$order,$params);
...
function collection_entries_header($c,$endpoint,$mode='multiple',$showCreate=true){
    $cid=(int)$c['id'];
    $back='<a class="btn btn-light collection-workspace-back" href="'.h(U(['collections'=>1])).'" aria-label="'.h(T('back')).'">'.icon('arrow-left').'</a>';
    $meta='<span class="badge text-bg-light">'.h(T($mode==='single'?'single':'multiple')).'</span>'.collection_sections_badges($cid);
    ...
    $actions='';
    if($showCreate&&can_entries()&&$mode!=='single')$actions.='<a class="btn btn-primary" href="'.h(U(['c'=>$cid,'entry'=>0])).'">'.icon('plus-lg').' '.h(T('new_entry')).'</a>';
    if(can_schema())$actions.='<a class="btn btn-light" href="'.h(U(['c'=>$cid,'fields'=>1])).'">'.icon('list-check').' '.h(T('fields')).'</a>';
    $items=[];
...
function single_rows($c){
    ...
    if(!$e)return $h.collection_entries_empty_state(T('no_entries'),T('create_first_entry'),can_entries()?'<a class="btn btn-primary" href="'.h(U(['c'=>$cid,'entry'=>0])).'">'.icon('plus-lg').' '.h(T('create_first_entry')).'</a>':'');
...
function rows($c){
    ...
    if(!$r&&$qv===''&&$status===''){return $h.collection_entries_empty_state(T('no_entries'),T('create_first_entry'),can_entries()?'<a class="btn btn-primary" href="'.h(U(['c'=>$cid,'entry'=>0])).'">'.icon('plus-lg').' '.h(T('create_first_entry')).'</a>':'');}
