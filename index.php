...
function form_notification_payload(array $form,int $submissionId,array $payload){$pr=project((int)$form['pid']);return ['event'=>'form.submitted','submission_id'=>$submissionId,'project'=>['id'=>(int)($pr['id']??0),'name'=>$pr['n']??'','slug'=>$pr['s']??''],'form'=>['id'=>(int)$form['id'],'name'=>$form['n'],'slug'=>$form['s']],'data'=>$payload,'received_at'=>now()];}
function dispatch_form_notifications(array $form,int $submissionId,array $payload){
    $event=form_notification_payload($form,$submissionId,$payload);$json=json_encode($event,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);$email=trim((string)($form['notify_email']??''));
...
function public_form_endpoint(){
    ...
    dispatch_form_notifications($f,$id,$payload);audit_event('form.submitted','form',(int)$f['id'],'Submission #'.$id,['submission_id'=>$id],$pid,null);$message=trim((string)($f['success_message']??''))?:T('form_default_success');J(['ok'=>true,'submission_id'=>$id,'message'=>$message],201);
}
...
function action(){
    ...
    try{
        ...
        if($a==='project'){
            ...
            $existingProject=$id?project($id):null;if($id){if(!$existingProject)throw new Exception(T('access_denied'));q('UPDATE p SET n=?,s=?,d=?,o=?,ua=? WHERE id=?',[$n,$ss,$d,$o,$tm,$id]);}
        ...
        if($a==='group'){
            require_perm(can_schema());$id=(int)($_POST['id']??0);$existing=$id?assert_group($id):null;$n=trim((string)($_POST['n']??''));$ss=unique_group_slug($_POST['s']?:$n,current_project_id(),$id);$d=trim((string)($_POST['d']??''));$o=(int)($_POST['o']??0);[$access,$keyHash,$keyEnc]=access_values_from_post($existing);$cors='*';$tm=now();if(!$n)throw new Exception(T('name_required'));
        ...
        if($a==='col'){
            require_perm(can_schema());$id=(int)($_POST['id']??0);$existing=$id?assert_collection($id):null;$n=trim((string)($_POST['n']??''));$ss=unique_collection_slug($_POST['s']?:$n,current_project_id(),$id);$d=trim((string)($_POST['d']??''));$o=(int)($_POST['o']??0);[$access,$keyHash,$keyEnc]=access_values_from_post($existing);$cors='*';$tm=now();if(!$n)throw new Exception(T('name_required'));
...
function collectionsPage(){
    ...
    $table='<table class="table table-hover align-middle mb-0 cms-responsive"><thead><tr><th>'.h(T('name')).'</th><th>'.h(T('type')).'</th><th>'.h(T('access_mode')).'</th><th>'.h(T('entry_count')).'</th><th>'.h(T('content_sections')).'</th><th>'.h(T('updated')).'</th><th class="text-end">'.h(T('actions')).'</th></tr></thead><tbody>';
    foreach($rows as $c){
...
function settingsPage(){
    ...
    if(can_api())$h.='<div class="col-12 col-lg-4"><div class="ios-surface p-4 h-100 d-flex flex-column"><div class="d-flex gap-3"><span class="btn btn-light btn-icon disabled">'.icon('key').'</span><div><h2 class="h5">'.h(T('api_keys')).'</h2><p class="text-muted">'.h(T('access_private_hint')).'</p></div></div><div class="mt-auto pt-4"><a class="btn btn-primary w-100" href="'.h(U(['api_keys'=>1])).'">'.icon('key').' '.h(T('api_management')).'</a></div></div></div>';
    if(is_admin_user()){
        $download=post_form('backup_project','<button class="btn btn-primary w-100">'.icon('download').' '.h(T('backup_project')).'</button>','class="mb-3"');
        $restore='<form method="post" enctype="multipart/form-data">'.token().'<input type="hidden" name="_a" value="restore_project_backup"><label class="form-label">'.h(T('restore_backup')).'</label><input class="form-control mb-2" type="file" name="backup" accept=".zip,application/zip" required><button class="btn btn-secondary w-100">'.icon('upload').' '.h(T('restore_backup')).'</button></form>';
        $h.='<div class="col-12 col-xl-8"><div class="ios-surface p-4 h-100"><div class="d-flex gap-3 mb-3"><span class="btn btn-light btn-icon disabled">'.icon('archive').'</span><div><h2 class="h5">'.h(T('backups')).'</h2><p class="text-muted">'.h(T('backup_hint')).'</p></div></div><div class="row g-3"><div class="col-12 col-lg-5">'.$download.'</div><div class="col-12 col-lg-7">'.$restore.'</div></div></div></div>';
        $h.='<div class="col-12 col-md-6 col-xl-2"><div class="ios-surface p-4 h-100 d-flex flex-column"><div><span class="btn btn-light btn-icon disabled mb-3">'.icon('activity').'</span><h2 class="h5">'.h(T('audit_log')).'</h2><p class="text-muted small">'.h(T('audit_hint')).'</p></div><div class="mt-auto"><a class="btn btn-secondary w-100" href="'.h(U(['audit'=>1])).'">'.h(T('open')).'</a></div></div></div>';
        $h.='<div class="col-12 col-md-6 col-xl-2"><div class="ios-surface p-4 h-100 d-flex flex-column"><div><span class="btn btn-light btn-icon disabled mb-3">'.icon('heart-pulse').'</span><h2 class="h5">'.h(T('diagnostics')).'</h2><p class="text-muted small">'.h(T('diagnostics_hint')).'</p></div><div class="mt-auto"><a class="btn btn-secondary w-100" href="'.h(U(['diagnostics'=>1])).'">'.h(T('open')).'</a></div></div></div>';
    }
...
https://admin.site.kz",'data-no-old'=>true]).'<div class="form-text mb-3">'.h(T('cors_origins_hint')).'<br>'.h(T('cors_default_all')).'</div><button class="btn btn-primary">'.icon('check-lg').' '.h(T('save')).'</button></form>';
    $h.='<div class="col-12"><div class="ios-surface p-4"><div class="d-flex gap-3 mb-3"><span class="btn btn-light btn-icon disabled">'.icon('shield-check').'</span><div><h2 class="h5">'.h(T('cors_settings')).'</h2><p class="text-muted">'.h(T('cors_settings_hint')).'</p></div></div>'.$corsForm.'</div></div>';
